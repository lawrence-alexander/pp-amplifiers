...
# Calculates Shannon entropy, alphanumeric bigram count, string length, vowel-consonant pattern as binary to decimal,
...
regex = False
regex_pattern = "[a-f][0-9]{2}|[0-9][a-f]{2}\g'0'"

def alphanumeric_bigrams(in_text):    
    count_bigrams = len(re.findall("[a-z][0-9]|[0-9][a-z]", in_text))
        ...
        if len(userident) >=2:         
                ...
                if counter == 0:
                    writer.writerow(["Username", "Username Entropy", "Username Length", "VC-bin-dec","VCR",
                                     "Digit Count","Digit Sum", "Alpha Bigrams"])
